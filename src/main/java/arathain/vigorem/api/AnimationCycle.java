...
public abstract class AnimationCycle {
	...
	protected void setMatrixTransform(MatrixStack s, Keyframe prev, Keyframe next, float tickDelta, boolean same) {
		if(same) {
			...
			s.multiply(Axis.X_POSITIVE.rotation(prev.rotation.getX()));
			s.multiply(Axis.Y_POSITIVE.rotation(prev.rotation.getY()));
			s.multiply(Axis.Z_POSITIVE.rotation(prev.rotation.getZ()));
			...
			s.multiply(Axis.X_POSITIVE.rotation(MathHelper.lerp(prev.easing.ease(percentage, 0, 1, 1), prev.rotation.getX(), next.rotation.getX())));
			s.multiply(Axis.Y_POSITIVE.rotation(MathHelper.lerp(prev.easing.ease(percentage, 0, 1, 1), prev.rotation.getY(), next.rotation.getY())));
			s.multiply(Axis.Z_POSITIVE.rotation(MathHelper.lerp(prev.easing.ease(percentage, 0, 1, 1), prev.rotation.getZ(), next.rotation.getZ())));
